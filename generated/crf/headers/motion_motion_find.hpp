...
inline void reset_motion_find(motion_find_state &state) {
    state = motion_find_state{};
}

inline void init_motion_find(motion_find_state &state, const robot_io &robot) {
...
inline void update_motion_find(
    motion_find_state &state,
    shared_data &shared,
    const robot_io &robot) {
    init_motion_find(state, robot);
...
inline bool can_start_motion_find(
    motion_find_state &state,
    shared_data &shared) {
    return true;
}

inline void control_motion_find(
    motion_find_state &state,
    shared_data &shared,
    const robot_io &robot) {
    ...
    shared.linvel_ee_z_err = motion_spec::runtime::evaluate_equality_constraint(shared.twist_ee_ee.vel[2], shared.linvel_zero_ref);
    // eval_cstr_angvel_find
    shared.angvel_ee_z_err_find = motion_spec::runtime::evaluate_equality_constraint(shared.twist_ee_ee.rot[2], shared.angvel_ee_z_ref_find);
    ...
    shared.eacc_ee_lin_x = state.ctrl_linvel_ee_x_zero.control(shared.linvel_ee_x_err);
    // ctrl_linvel_ee_y_zero
    shared.eacc_ee_lin_y = state.ctrl_linvel_ee_y_zero.control(shared.linvel_ee_y_err);
    // ctrl_angvel_ee_x_zero
    shared.eacc_ee_ang_x = state.ctrl_angvel_ee_x_zero.control(shared.angvel_ee_x_err);
    // ctrl_angvel_ee_y_zero
    shared.eacc_ee_ang_y = state.ctrl_angvel_ee_y_zero.control(shared.angvel_ee_y_err);
    // ctrl_angvel_find
    shared.eacc_ee_ang_z_find = state.ctrl_angvel_find.control(shared.angvel_ee_z_err_find);
    // ctrl_linvel_ee_z_zero
    shared.eacc_ee_lin_z = state.ctrl_linvel_ee_z_zero.control(shared.linvel_ee_z_err);




    {
        const bool active = motion_spec::runtime::constraint_satisfied(shared.cstr_torque_find_shutoff_err);
        if (motion_spec::runtime::rising_edge(state.mon_torque_find_shutoff_previous, active)) {
            motion_spec::runtime::warn_produce_event_not_implemented("evt_cstr_torque_find_shutoff");
        }
    }
    ...
    {
        const bool active = motion_spec::runtime::constraint_satisfied(shared.cstr_rotation_find_limit_err);
        if (motion_spec::runtime::rising_edge(state.mon_rotation_find_limit_previous, active)) {
            motion_spec::runtime::warn_produce_event_not_implemented("evt_cstr_rotation_find_limit");
        }
    }
    ...
    {
        const bool active = motion_spec::runtime::constraint_satisfied(shared.cstr_torque_find_upper_limit_err);
        if (motion_spec::runtime::rising_edge(state.mon_torque_find_upper_limit_previous, active)) {
            motion_spec::runtime::warn_produce_event_not_implemented("evt_cstr_torque_find_upper_limit");
        }
    }
