...
- [x] Phase 6 — 6a/6b/6c done (verified below); 6d/6e assessed and deliberately not done.
  - **6a** twist/pose-diff/wrench constructor dedup (`_spatial_coordinate_fields`), 3 concepts kept. Byte-identical, pytest 27.
  - **6b** MonitorEntry → LevelMonitor | EdgeMonitor (disjoint fields; StringTemplate/`.get`/getattr make missing==None). pytest 27 + pick_place S_DONE @ 22308.
  - **6c** Controller → PIDController | ImpedanceController | FeedForwardController (RDF-typed; template already dispatched on `.type`). pytest 27 + pick_place (PID) S_DONE @ 22308 + admittance (impedance) S_DONE @ 17470.
  - **6d role-flag extraction — DONE:** lifted `authored`+`snapshot` (universal across all 9 quantity types) into a `Provenance` dataclass the quantities reference (`provenance: Provenance`). Reads (codegen.py:636/742 declared-pose check; ir_gen introspection contract) go through `provenance`. `has_view` (only 4 types, structural graph-shape fact, read once at ir_gen:2614 for Quantity) is a *different* concern — kept as a direct field, not folded into provenance. Verified: pytest 27; pick_place S_DONE @ 22308 (FSM identical); introspection contract still carries authored (92 true) after fixing the getattr read.
  - **6e — not done (assessed as legitimate, confirmed):** `is_elapsed` IS required — pick_place_single_wait produces `is_elapsed: true` + `elapsed_threshold_s`, read at ~11 codegen sites to generate the "wait N s" timing check; it's a modifier flag, not a removable/splittable concept. `Wrench.sensor_name` and `EdgeMonitor.debounce_steps` (cached derived, needed by templates) are legitimate fields.
