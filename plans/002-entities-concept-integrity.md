...
- [x] Phase 6 — 6a/6b/6c done (verified below); 6d/6e assessed and deliberately not done.
  ...
  - **6d role-flag extraction — NOT DONE (assessed as non-improvement):** authored/snapshot/has_view are read only in Python (9 sites, 0 template uses) and are genuine per-quantity properties. Nesting into a `role` object adds indirection without removing per-instance data; a shared base is fiddly (default-ordering vs the leading non-default `id`). Churn, not clarity.
  - **6e foreign-concern lifts — NOT DONE (assessed as legitimate):** `is_elapsed` is a boolean modifier gating ~11 codegen sites (acceptable flag, not a concept); `Wrench.sensor_name` and `EdgeMonitor.debounce_steps` (cached derived, needed by templates) are legitimate fields.
