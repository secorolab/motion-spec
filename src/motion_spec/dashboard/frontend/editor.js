...
/** CodeMirror, imported once, themed to the page, and told which lines changed since HEAD. */

import { $, post, snack } from "./core.js";

// The import promise, kept so a second file does not fetch the library again.
let codemirror = null;

// The view on screen, so anything pointing at a line of the open file can jump to it.
let mounted = null;
...
// Vendored, not fetched: the dashboard runs beside a robot, often with no way out to a CDN.
// `scripts/vendor_frontend.py` mirrors these and pins the versions.
//
// Each has to be the exact file `codemirror` itself imports, or a second copy of the package
// is loaded, whose decorations the first copy quietly ignores.
const CM_VIEW = "./vendor/esm/cm-view.mjs";

const CM_SEARCH = "./vendor/esm/cm-search.mjs";

const CM_LANGUAGE = "./vendor/esm/cm-language.mjs";

// The tag vocabulary a highlight style is written against; the language package does not
// re-export it.
const CM_TAGS = "./vendor/esm/lezer-highlight.mjs";

// Legacy modes, not the lang-* packs: a lang-* pack builds its LanguageSupport out of its own
// copy of @codemirror/language, which the copy basicSetup uses ignores, and the file then
// renders with no highlighting at all.
...
// A pack this machine cannot fetch falls back to the structural mode rather than refusing.
async function languageFor(name, StreamLanguage) {
  ...
  if (!packs.has(wanted.url)) {
    packs.set(
      wanted.url,
      import(wanted.url).then(wanted.parser).catch(() => null),
    );
  }
...
// Tokyo Night. Code wants more colours than the page's own one-accent palette has.
const TOKYO_NIGHT = {
  comment: "#565f89",
  red: "#f7768e",
  orange: "#ff9e64",
...
function tokyoNightStyle(HighlightStyle, tags) {
...
// Highlighting by shape, not vocabulary: one mode for every DSL here, since what they share
// is punctuation. No per-language keyword list to go stale when a grammar gains a term.
function structuralMode(StreamLanguage) {
...
// Which lines of `b` are additions or changes against `a`, by Myers' O(ND) shortest-edit
// script. Its cost tracks the number of differences, not the file size, so a small edit stays
// cheap in a large file.
function myersInsertions(a, b) {
  ...
  for (let d = dEnd; d > 0; d -= 1) {
    ...
    const k = x - y;
    const prevK =
      k === -d || (k !== d && (vPrev.get(k - 1) ?? -1) < (vPrev.get(k + 1) ?? -1)) ? k + 1 : k - 1;
...
export async function mountEditor(
  ...
  // The button arrives with the first edit: there is nothing to save before one.
  const dirty = (is) => {
    if (!save) return;
    save.hidden = !is;
  ...
  const write = async (target) => {
    ...
    try {
      ...
      // Still shown: an uncommitted save is a change from git HEAD's point of view.
      save.hidden = false;
      save.textContent = "saved";
      setTimeout(() => dirty(false), 1200);
      // The file is written either way; a model that no longer parses is worth saying.
  ...
  // Where the parse gave up. The next edit clears it rather than leaving a stale mark on a
  // line that has since moved.
  const badLine = Decoration.line({ class: "cm-badLine" });
  const problem = { line: null };
  const badMarks = (doc) =>
    problem.line && problem.line <= doc.lines
  ...
  // Measured against git HEAD, so a file already dirty on disk shows that when it opens and
  // only a commit clears the marks. With no commit (new file, no repo, no git) the open-time
  // text is the baseline, which marks nothing.
  const was = (gitHead ?? text).split("\n");
  const changedLine = Decoration.line({ class: "cm-changedLine" });
  const marked = (doc) => {
    const now = [];
    for (let n = 1; n <= doc.lines; n += 1) now.push(doc.line(n).text);
    // Trimming the matching ends first keeps what Myers has to search small in the common
    // case of one edit in an otherwise-untouched file.
    let head = 0;
  ...
  const changes = (doc) =>
    Decoration.set(
      [...marked(doc)]
        ...
        .map((n) => changedLine.range(doc.line(n).from)),
    );
  const trackChanges = ViewPlugin.fromClass(
    class {
      ...
      // The baseline is fixed for the life of this editor, so scrolling is not worth a redraw.
      update(target) {
      ...
      EditorView.theme(
        {
          ...
          // Translucent: the selection is drawn behind the lines, so an opaque active line
          // would hide every selection made inside it.
          ".cm-activeLine": { backgroundColor: "rgba(255, 255, 255, .06)" },
          ".cm-changedLine": { boxShadow: "inset 2px 0 0 var(--accent)" },
          // Tinted, not edge-marked, so it does not read as another uncommitted change.
          ...
          // A box of no height, stuck to the top, so the panel floats in the corner instead of
          // taking a bar across the text it is searching.
          ".cm-panels": {
            position: "sticky",
            top: "var(--head, 60px)",
            zIndex: "2",
          ...
          // Seven columns because the search extension's first row has seven controls. A grid,
          // not a flex row: a shrink-to-fit flex box measures as if nothing wrapped, and the
          // panel's own <br> then never breaks.
          ".cm-panel.cm-search": {
            position: "absolute",
            top: "8px",
            right: "0",
          ...
          // A native checkbox is a white box whatever the page around it is.
          ...
          // A hidden element is not a grid item, so the replace field starts row two by itself.
  ...
  // Tokenise the whole file now, or scrolling shows a frame of plain text before the colours.
  // These files are hundreds of lines and take single-digit milliseconds; the budget is the
  // backstop for a bigger one, which then highlights as it goes.
