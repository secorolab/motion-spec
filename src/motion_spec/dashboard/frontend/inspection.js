...
const read = (kind, path) => {
  try {
    return JSON.parse(localStorage.getItem(key(kind, path)));
  } catch {
    return null;
  }
};
...
function snapshot() {
  return {
    ...
    plots: [...document.querySelectorAll("#plots .plot-card")]
      .map((card) => card.inspection?.())
      .filter(Boolean),
  };
}

/** Saved only for an archived replay, never a pending or streaming page. */
export function saveInspection() {
  if (
    ...
    state.following ||
    state.restoringInspection
  )
    return;
  try {
    localStorage.setItem(key("inspection", state.runPath), JSON.stringify(snapshot()));
  } catch {
    snack("Browser storage is full; inspection view was not saved");
  }
}
...
function applyView(view, preset = false) {
  ...
  try {
    ...
    const recorded = new Set(state.replay.signals);
    const missing = new Set();
    for (const plot of view.plots) {
      const signals = plot.signals.filter((signal) => {
        if (!recorded.has(signal)) missing.add(signal);
        return recorded.has(signal);
      });
    ...
    if (!preset) {
      ...
      const camera = [...document.querySelectorAll(".video-pick")].find(
        (pick) => pick.dataset.camera === view.camera,
      );
      camera?.click();
      const panel = [...document.querySelectorAll(".replay-tabs button")].find(
        (button) => button.dataset.panel === view.panel,
      );
      panel?.click();
    }
    if (missing.size) snack(`Unavailable signals skipped: ${[...missing].join(", ")}`);
  } finally {
    state.restoringInspection = false;
  }
}
...
export function restoreInspection(path) {
  ...
  if (url.has("panel")) {
    [...document.querySelectorAll(".replay-tabs button")]
      .find((button) => button.dataset.panel === url.get("panel"))
      ?.click();
  }
}
