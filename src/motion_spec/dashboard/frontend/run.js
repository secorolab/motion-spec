...
export function bindPanels() {
  ...
  // a reload lands back where it left off, like the run it reopens; a hash naming a panel
  // this page no longer offers falls back to plots
  const stored = new URLSearchParams(location.hash.slice(1)).get("panel");
  show(["plots", "sparql", "console"].includes(stored) ? stored : "plots", false);
}
