...
def test_organize_annotate_and_resume(dashboard):  # noqa: F811 -- pytest fixture
    ...
    with playwright.sync_playwright() as driver:
        ...
        page.locator("#browser .item").first.click()
        playwright.expect(page.locator(".generation .path")).to_be_visible()
        opened = page.locator(".generation .path").inner_text()
        playwright.expect(page.locator("#content")).to_have_css("padding-bottom", "24px")
        # Switching generations writes into the page that is up; a rebuilt one loses this mark.
        page.evaluate("document.querySelector('#content .generation').dataset.mark = 'kept'")
        page.locator("#browser .item").last.click()
        playwright.expect(page.locator(".generation .path")).not_to_have_text(opened)
        playwright.expect(page.locator("#content .generation")).to_have_attribute("data-mark", "kept")
        playwright.expect(page.locator(".generation-notes")).to_have_count(1)
        editor = page.locator(".annotation-editor")
        playwright.expect(editor).to_have_count(1)
        ...
        playwright.expect(page.get_by_label("Plot preset", exact=True)).to_have_text("Contact view")
        playwright.expect(page.locator("select.presets")).to_have_value("Contact view")
        page.reload()
        playwright.expect(page.locator(".plot-card .plot-signal")).to_have_count(1)
        page.locator(".remove-plot").click()
        page.locator("select.presets").select_option("Contact view")
