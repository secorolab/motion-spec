...
def test_the_platform_says_whether_a_display_is_a_choice(tmp_path):
    assert server.is_simulated(_generation(tmp_path / "sim", simulated=True))
    assert not server.is_simulated(_generation(tmp_path / "real", simulated=False))
    # a directory that is no generation at all claims nothing
    assert not server.is_simulated(tmp_path / "nowhere")


def test_every_endpoint_the_config_names_is_what_the_data_says_it_is(tmp_path):
    assert server.device_endpoints(_toml(tmp_path, ROBOT_TOML)) == [
        {
...
def test_a_topic_or_a_pose_is_nothing_to_connect_to(tmp_path):
    named = {endpoint["name"] for endpoint in server.device_endpoints(_toml(tmp_path, ROBOT_TOML))}
...
def test_a_host_with_no_port_is_still_a_device_to_name(tmp_path):
    endpoints = server.device_endpoints(_toml(tmp_path, '[arm.base]\nip = "10.0.0.2"\n'))
    assert endpoints == [
        {
            "name": "arm.base",
            "kind": "network",
...
def test_a_generation_that_archived_no_config_has_nothing_to_probe(tmp_path):
    ...
    assert server.probe_devices(generation) == {"devices": [], "config": None}
...
def test_a_port_something_listens_on_answers(tmp_path):
    with socket.create_server(("127.0.0.1", 0)) as listener:
        ...
        report = server.probe_devices(generation)
...
def test_a_port_nothing_listens_on_says_why(tmp_path):
    ...
    device = server.probe_devices(generation)["devices"][0]
...
def test_a_probe_never_carries_what_the_config_logs_in_with(tmp_path):
    ...
    assert "hunter2" not in repr(server.probe_devices(generation))
...
def _health(monkeypatch, checks, *, delay=0.0):
    ...
    monkeypatch.setattr(server, "HEALTH", {"checks": None, "stamp": None, "thread": None})
...
def test_the_checks_are_run_once_and_then_remembered(monkeypatch):
    ...
    assert server.health_report() == {"running": True, "checks": None, "stamp": None}
    server.HEALTH["thread"].join(5)
    ...
    assert not server.HEALTH["thread"].is_alive()
...
def test_a_re_check_runs_them_again(monkeypatch):
    ...
    server.HEALTH["thread"].join(5)
    first = server.HEALTH["thread"]
    server.health_report(refresh=True)
    assert server.HEALTH["thread"] is not first
...
def test_a_report_asked_for_while_they_run_says_so(monkeypatch):
    ...
    server.health_report()
    assert server.health_report() == {"running": True, "checks": None, "stamp": None}
    assert isinstance(server.HEALTH["thread"], threading.Thread)
    server.HEALTH["thread"].join(5)
